...
type AttachAsadmCmd struct {
	...
	Help            AttachAsadmCmdHelp     `command:"help" subcommands-optional:"true" description:"Print help"`
}

type AttachAsadmCmdHelp struct{}

func (c *AttachAsadmCmdHelp) Execute(args []string) error {
	PrintHelp(false, "To specify a command to run, use the following syntax:\n\n  aerolab attach asadm <parameters> -- <asadm-parameters>\n\nFor example:\n\n  aerolab attach asadm --name=bob -- -e info\n")
	return nil
}

func (c *AttachAsadmCmd) Execute(args []string) error {
	cmd := []string{"attach", "asadm"}
	...
	err = c.AttachAsadm(system, system.Backend.GetInventory(), args)
	if err != nil {
		return Error(err, system, cmd, c, args)
	}
...
func (c *AttachAsadmCmd) AttachAsadm(system *System, inventory *backends.Inventory, args []string) error {
	if system == nil {
		var err error
		system, err = Initialize(&Init{InitBackend: true, ExistingInventory: inventory}, []string{"attach", "asadm"}, c)
		if err != nil {
			return err
		}
	}

...
func (c *AerospikeRestartCmd) Execute(args []string) error {
	...
	instances, err := c.RestartAerospike(system, system.Backend.GetInventory(), system.Logger, args, "restart")
	if err != nil {
		return Error(err, system, cmd, c, args)
	}
	system.Logger.Info("Restarted aerospike on %d instances", instances.Count())
...
func (c *AerospikeRestartCmd) RestartAerospike(system *System, inventory *backends.Inventory, logger *logger.Logger, args []string, action string) (backends.InstanceList, error) {
	if system == nil {
		var err error
	...
	if strings.Contains(c.ClusterName.String(), ",") {
		...
		for _, cluster := range clusters {
			...
			inst, err := c.RestartAerospike(system, inventory, logger, args, action)
			if err != nil {
				return nil, err
			}
	...
	logger.Info("Restarting aerospike on %d nodes", cluster.Count())

	// Execute systemctl restart aerospike on all instances
	out := cluster.Exec(&backends.ExecInput{
		ExecDetail: sshexec.ExecDetail{
			Command:        []string{"systemctl", "restart", "aerospike"},

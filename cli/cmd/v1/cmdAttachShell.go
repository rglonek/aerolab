...
type AttachShellCmd struct {
	...
	Help            AttachShellCmdHelp     `command:"help" subcommands-optional:"true" description:"Print help"`
}

type AttachShellCmdHelp struct{}

func (c *AttachShellCmdHelp) Execute(args []string) error {
	PrintHelp(false, "To specify a command to run, use the following syntax:\n\n  aerolab attach shell <parameters> -- <command>\n\nFor example:\n\n  aerolab attach shell --name=bob -- ls -l /tmp\n")
	return nil
}

func (c *AttachShellCmd) Execute(args []string) error {
	cmd := []string{"attach", "shell"}
	...
	err = c.AttachShell(system, system.Backend.GetInventory(), args)
	if err != nil {
		return Error(err, system, cmd, c, args)
	}
...
func (c *AttachShellCmd) AttachShell(system *System, inventory *backends.Inventory, args []string) error {
	if system == nil {
		var err error
		system, err = Initialize(&Init{InitBackend: true, ExistingInventory: inventory}, []string{"attach", "shell"}, c)
		if err != nil {
			return err
		}
	}

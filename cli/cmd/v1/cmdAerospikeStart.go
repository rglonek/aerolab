...
func (c *AerospikeStartCmd) Execute(args []string) error {
	...
	instances, err := c.StartAerospike(system, system.Backend.GetInventory(), system.Logger, args, "start")
	if err != nil {
		return Error(err, system, cmd, c, args)
	}
	system.Logger.Info("Started aerospike on %d instances", instances.Count())
...
func (c *AerospikeStartCmd) StartAerospike(system *System, inventory *backends.Inventory, logger *logger.Logger, args []string, action string) (backends.InstanceList, error) {
	if system == nil {
		var err error
	...
	if strings.Contains(c.ClusterName.String(), ",") {
		...
		for _, cluster := range clusters {
			...
			inst, err := c.StartAerospike(system, inventory, logger, args, action)
			if err != nil {
				return nil, err
			}
	...
	logger.Info("Starting aerospike on %d nodes", cluster.Count())

	// Execute systemctl start aerospike on all instances

...
type AttachAsinfoCmd struct {
	...
	Help            AttachAsinfoCmdHelp    `command:"help" subcommands-optional:"true" description:"Print help"`
}

type AttachAsinfoCmdHelp struct{}

func (c *AttachAsinfoCmdHelp) Execute(args []string) error {
	PrintHelp(false, "To specify a command to run, use the following syntax:\n\n  aerolab attach asinfo <parameters> -- <asinfo-parameters>\n\nFor example:\n\n  aerolab attach asinfo --name=bob -- -v service\n")
	return nil
}

func (c *AttachAsinfoCmd) Execute(args []string) error {
	cmd := []string{"attach", "asinfo"}
	...
	err = c.AttachAsinfo(system, system.Backend.GetInventory(), args)
	if err != nil {
		return Error(err, system, cmd, c, args)
	}
...
func (c *AttachAsinfoCmd) AttachAsinfo(system *System, inventory *backends.Inventory, args []string) error {
	if system == nil {
		var err error
		system, err = Initialize(&Init{InitBackend: true, ExistingInventory: inventory}, []string{"attach", "asinfo"}, c)
		if err != nil {
			return err
		}
	}
	...
	args = append([]string{"asinfo"}, args...)

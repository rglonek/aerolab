...
type AttachAqlCmd struct {
	...
	Help            AttachAqlCmdHelp       `command:"help" subcommands-optional:"true" description:"Print help"`
}

type AttachAqlCmdHelp struct{}

func (c *AttachAqlCmdHelp) Execute(args []string) error {
	PrintHelp(false, "To specify a command to run, use the following syntax:\n\n  aerolab attach aql <parameters> -- <aql-parameters>\n\nFor example:\n\n  aerolab attach aql --name=bob -- -c 'SELECT * from test;'\n")
	return nil
}

func (c *AttachAqlCmd) Execute(args []string) error {
	cmd := []string{"attach", "aql"}
	...
	err = c.AttachAql(system, system.Backend.GetInventory(), args)
	if err != nil {
		return Error(err, system, cmd, c, args)
	}
...
func (c *AttachAqlCmd) AttachAql(system *System, inventory *backends.Inventory, args []string) error {
	if system == nil {
		var err error
		system, err = Initialize(&Init{InitBackend: true, ExistingInventory: inventory}, []string{"attach", "aql"}, c)
		if err != nil {
			return err
		}
	}

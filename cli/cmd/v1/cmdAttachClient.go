...
type AttachClientCmd struct {
	...
	Help            AttachClientCmdHelp    `command:"help" subcommands-optional:"true" description:"Print help"`
}

type AttachClientCmdHelp struct{}

func (c *AttachClientCmdHelp) Execute(args []string) error {
	PrintHelp(false, "To specify a command to run, use the following syntax:\n\n  aerolab attach client <parameters> -- <command>\n\nFor example:\n\n  aerolab attach client --name=bob -- ls -l /tmp\n")
	return nil
}

func (c *AttachClientCmd) Execute(args []string) error {
	cmd := []string{"attach", "client"}
	...
	err = c.AttachClient(system, system.Backend.GetInventory(), args)
	if err != nil {
		return Error(err, system, cmd, c, args)
	}
...
func (c *AttachClientCmd) AttachClient(system *System, inventory *backends.Inventory, args []string) error {
	if system == nil {
		var err error
		system, err = Initialize(&Init{InitBackend: true, ExistingInventory: inventory}, []string{"attach", "client"}, c)
		if err != nil {
			return err
		}
	}
	...
	attach := &InstancesAttachCmd{
		...
		Filters: InstancesListFilter{
			...
			NodeNo:      node,
			Tags:        []string{"aerolab.old.type=client"},
		},
	}

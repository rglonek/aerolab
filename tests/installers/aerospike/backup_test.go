...
func backupScript(t *testing.T) aerospike.Files {
	products, err := aerospike.GetProducts(time.Second * 10)
	require.NoError(t, err)
	require.NotNil(t, products)
	require.NotEmpty(t, products)
	product := products.WithName("aerospike-backup-service")
...
func Test01_BackupScriptDocker(t *testing.T) {
	files := backupScript(t)
	d, e := files.GetInstallScript(aerospike.ArchitectureTypeX86_64, aerospike.OSName("ubuntu"), "24.04", true, true, true, true)
	require.NoError(t, e)
	require.NotNil(t, d)
